 ...
 * offer any privacy for properties or methods, but it's useful for eliminating anonymous
 * functions from your code, centralizing configuration options, and easing the path to
 ...
 * property and several methods. All of the properties and methods are public, so any part
 * of your application can see the properties and call methods on the object. While there
 * is an init method, there's nothing requiring that it be called before the object is
 * functional.
 * 
 * How would we apply this pattern to jQuery code? Let's say that we had this code written
 * in the traditional jQuery style:
 */
...
/**
 * If this were the extent of our application, leaving it as-is would be fine. On the other
 * hand, if this was a piece of a larger application, we'd do well to keep this functionality
 * separate from unrelated functionality. We might also want to move the URL out of the code
 * and into a configuration area. Finally, we might want to break up the chain to make it
 * easier to modify pieces of the functionality later.
 */
...
/**
 * The first thing you'll notice is that this approach is obviously far longer than the
 * original — again, if this were the extent of our application, using an object literal
 * would likely be overkill. Assuming it's not the extent of our application, though, we've
 * gained several things:
 * 
 * -> We've broken our feature up into tiny methods. In the future, if we want to change
 *    how content is shown, it's clear where to change it. In the original code, this step
 ...
 * remix, and rearrange.
 * 
 * For non-trivial features, object literals are a clear improvement over a long stretch of
 * code stuffed in a $(document).ready() block, as they get us thinking about the pieces
 * of our functionality. However, they aren't a whole lot more advanced than simply having a
 * bunch of function declarations inside of that $(document).ready() block.
 */

/** 
 * The Module Pattern
 * 
 * The module pattern overcomes some of the limitations of the object literal, offering
 * privacy for variables and functions while exposing a public API if desired.
 */
...
/**
 * In the example above, we self-execute an anonymous function that returns an object. Inside
 * of the function, we define some variables. Because the variables are defined inside of the
 * function, we don't have access to them outside of the function unless we put them in the
 * return object. This means that no code outside of the function has access to the
 * 'privateThing' variable or to the 'changePrivateThing' function. However, 'sayPrivateThing'
 * does have access to 'privateThing' and 'changePrivateThing', because both were defined in
 * the same scope as 'sayPrivateThing'.
 * 
 ...
 * feature using the module pattern while only exposing one public method of the module,
 * showItemByIndex().
 */
